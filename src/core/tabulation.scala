...
object Column:
  def apply[Row, V: AnsiShow](name: String,
                              getter: Row => V,
                              width: Width = Width.Flexible,
                              align: Align = Align.Left): Column[Row] =
    new Column[Row](name, width, align) { def get(r: Row): String = summon[AnsiShow[V]].show(getter(r)) }

abstract class Column[Row](val name: String,
                           val width: Width = Width.Flexible,
                           val align: Align = Align.Left):
  def get(r: Row): String

case class Table[Row](headings: Column[Row]*):
  ...
  def tabulate(maxWidth: Int, rows: Seq[Row], ansi: Option[String] = None): Seq[String] =
    ...
    val maxWidths: Vector[Int] = data.foldLeft(Vector.fill(headings.size)(0)) {
      case (widths, next) =>
        widths.zip(next).map { case (w, xs) => xs.map(Ansi.strip(_).length).max max w }
    }
    ...
    val initialWidths = headings.zip(maxWidths).foldLeft((Vector[Int](), maxWidth, flexibleWidths)) {
    ...
    List(startHr) ++ data.flatMap { cells =>
      ...
      }.transpose.map(_.mkString(s"${ansi.getOrElse("")}${if tight then "│" else "║"}${reset} ",
          s" ${ansi.getOrElse("")}│${reset} ", s" ${ansi.getOrElse("")}${if tight then "│" else "║"}${reset}"))
  ...
  private def pad(str: String, width: Int, alignment: Align, reset: String): String =
    ...
    alignment match
      case Align.Left =>
        if stripped > width
        then str.dropRight(stripped - width)+reset
        else str + (" " * (width - stripped))
      case Align.Right =>
        if stripped > width
        then str.drop(stripped - width)+reset
